...
def test_classify():
    import vision
    assert vision.classify(_hand([1, 1, 1, 1], True))[0] == "OPEN"
    assert vision.classify(_hand([1, 1, 1, 1], False))[0] == "OPEN"
    assert vision.classify(_hand([1, 0, 0, 0], False))[0] == "INDEX"
    assert vision.classify(_hand([0, 0, 0, 0], True))[0] == "THUMB"
    assert vision.classify(_hand([0, 0, 0, 0], False))[0] == "FIST"
    assert vision.classify(_hand([1, 1, 0, 0], False))[0] == "NONE", "dois dedos nao e gesto"
    assert vision.classify(_hand([1, 0, 0, 0], True))[0] == "NONE", "indicador + polegar nao e gesto"


def test_debounce():
    import vision
    d = vision.Debouncer(hold=0.2)
...
def test_gesture_edge():
    from main import GestureInput
    gi = GestureInput()
    jumps = [gi.read(s).jump for s in ("OPEN", "FIST", "FIST", "OPEN", "FIST", "NONE", "FIST")]
